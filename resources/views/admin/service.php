	...
	<div class="list clear">';
		...
		if(mysql_num_rows($res)) {
			echo '<div class="head row">
				...
				<div class="col col-xs-2">Category</div>
...
if ($do == 'add') {
	echo '<form action="'.$page.'.php?do=new" method="post" name="form1" target="_self" enctype="multipart/form-data">
		...
		<label>
			<div class="signature">Краткое описание:</div>
			<div class="field">
...
if ($do == 'new') {
	if(mysql_query ("INSERT INTO ".$page." (active, category, title, description, text) VALUES ('".$val['active']."', '".$val['category']."', '".$val['title']."', '".$val['description']."', '".$val['text']."')")) 
...
if ($do == 'update') { 		
	if(mysql_query("UPDATE ".$page." SET active='".$val['active']."', category='".$val['category']."', title='".$val['title']."', description='".$val['description']."', text='".$val['text']."' WHERE id='".$id."'")) 
